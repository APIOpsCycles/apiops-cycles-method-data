...
function printUsage() {
  ...
  node packages/create-apiops/bin/method-cli.js document [--extension integration-design | --cycle <cycle-id>] [--format confluence-wiki|markdown|adr] [--locale <locale>] [--title <title>] [--output <file>]
...
Examples:
  ...
  node packages/create-apiops/bin/method-cli.js document --cycle capability-productization-cycle --format markdown --output ./specs/capability-cycle.md
`);
}
...
function runDocumentCommand(options) {
  ...
  if (options.cycle) {
    if (!["confluence-wiki", "markdown", "adr"].includes(format)) {
      fail(`Unsupported cycle document format: ${format}`);
    }
    const renderer = format === "adr"
      ? methodEngine.renderCycleAdrMarkdown
      : format === "markdown"
        ? methodEngine.renderCycleMarkdown
        : methodEngine.renderCycleConfluenceWiki;
    ...
    console.log(`${format} cycle document: ${outputPath}`);
    return;
  }
