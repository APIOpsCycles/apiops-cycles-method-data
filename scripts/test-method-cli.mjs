...
try {
  ...
  assert(
    generatedDocument.includes("h3. Capability Strategy"),
    "Expected generated document to group canvases under their station."
  );
  assert(
    generatedDocument.indexOf("h3. Capability Strategy") === generatedDocument.lastIndexOf("h3. Capability Strategy"),
    "Expected station guidance to be shown once for canvases in the same station."
  );
  assert(
    generatedDocument.includes("h3. Consumer Requirements & Onboarding"),
    "Expected consumer experience canvases to be grouped under their station."
  );
  assert(
    generatedDocument.includes("h3. Architecture & Platform Decisions"),
  ...
  assert(
    generatedDocument.includes("Station description: Frame the business need as a reusable capability before choosing the implementation style."),
    "Expected core capability station description text to come from labels.stations.json."
  );
  assert(
    generatedDocument.includes("Station instruction: Map the customer or partner journey that creates the capability need"),
    "Expected core capability station instruction text to come from labels.stations.json."
  );
  assert(
  ...
  const capabilityCycleDocument = execFileSync(
    process.execPath,
    [
      methodCliPath,
      "document",
  ...
  assert(
    capabilityCycleDocument.includes("# Capability Productization Cycle"),
    "Expected capability cycle markdown output."
  );
  assert(
    capabilityCycleDocument.includes("Capability Strategy -> Consumer Requirements & Onboarding -> Architecture & Platform Decisions"),
    "Expected capability cycle to use capability-oriented station labels."
  );
  assert(
    capabilityCycleDocument.includes("| Topic | Question | Suggested answer owner | Answer | Confidence | Evidence | Open questions |"),
    "Expected cycle question list to include answer, confidence, evidence, and open-question columns."
  );
  assert(
    capabilityCycleDocument.includes("Expert answers are inputs to architecture and design decisions, not final design decisions."),
    "Expected cycle document to remind users that expert answers are decision inputs."
  );
  const apiCycleDocument = execFileSync(
    process.execPath,
    [
      methodCliPath,
      "document",
  ...
  assert(
    apiCycleDocument.includes("API Product Strategy -> API Consumer Experience -> API Platform Architecture"),
    "Expected API cycle to preserve API-specific station labels."
  );
  const integrationCycleDocument = execFileSync(
    process.execPath,
    [
      methodCliPath,
      "document",
      "--cycle", "integration-productization-cycle",
      "--format", "markdown",
      "--locale", "en"
    ],
    {
  ...
  assert(
    integrationCycleDocument.includes("Integration Capability Strategy -> Integration Consumer Requirements & Onboarding -> Integration Architecture Decision"),
    "Expected integration cycle to use integration-specific station labels."
  );
  const adrDocument = execFileSync(
    process.execPath,
    [
      methodCliPath,
      "document",
  ...
  assert(
    adrDocument.includes("# ADR: Integration Productization Cycle"),
    "Expected ADR template for cycle documents."
  );
